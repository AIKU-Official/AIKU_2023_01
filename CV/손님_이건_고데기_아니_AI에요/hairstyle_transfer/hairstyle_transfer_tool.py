...
import utils_hairstyle, os, torch, imageio, numpy as np, cv2, string, pickle as pkl

# * merge.py에서 import해서 실행하므로 경로 추가.
    ...
    def hairstyle_transfer(self, im1_path, im2_path, alpha_blend=True):
        ...
        if os.path.exists(thefilename):
            ...
            if self.net is None:
                ...
                if alpha_blend:
                    inner_face1, hair1, mask1 = utils_hairstyle.load_and_split_image(im1_path, return_mask=True)
                    inner_face2, hair2, mask2 = utils_hairstyle.load_and_split_image(im2_path, return_mask=True)
                else:
                    inner_face1, hair1 = utils_hairstyle.load_and_split_image(im1_path)
                inner_face2, hair2 = utils_hairstyle.load_and_split_image(im2_path)
            # print(f"Range of inner face: [{inner_face2.min()}, {inner_face2.max()}]")
            im2_orig = inner_face2 + hair2
            input_face = utils_hairstyle.numpy_uint8_to_torch(inner_face2.astype(np.uint8))
            input_hair = utils_hairstyle.numpy_uint8_to_torch(hair1.astype(np.uint8))
            if alpha_blend:
                masks = [
                 mask2]
                im_orig = [im2_orig]
            ...
            for ii in range(output_images.shape[0]):
                img_generated = utils_hairstyle.torch_to_numpy_uint8(output_images[ii:ii + 1])
                if alpha_blend:
                    img_original = im_orig[ii]
                    im_mask = utils_hairstyle.gaussian_filter((masks[ii]), sigma=(self.alpha_blend_sigma))
                    ...
                    masked_output = utils_hairstyle.alpha_blend_images(img_generated, img_original, im_mask)
    ...
    def interpolation_single_pair(self, im1_path, im2_path, n_steps=10, interpolate_hair=True, alpha_blend=True):
        if os.path.exists(self.__prepare_filename(im1_path, im2_path, alpha_blend, kwargs={'phase':'interp',  'hair':interpolate_hair,  'curr_step':0,  'max_step':n_steps})):
            ...
            if self.net is None:
                ...
                im2_orig = inner_face2 + hair2
                inner_face1_t = utils_hairstyle.numpy_uint8_to_torch(inner_face1.astype(np.uint8))
                inner_face2_t = utils_hairstyle.numpy_uint8_to_torch(inner_face2.astype(np.uint8))
                hair1_t = utils_hairstyle.numpy_uint8_to_torch(hair1.astype(np.uint8))
                hair2_t = utils_hairstyle.numpy_uint8_to_torch(hair2.astype(np.uint8))
                if interpolate_hair:
                ...
                with torch.no_grad():
                    if interpolate_hair:
                        ...
                        end_result_batch = self.net.forward_v_i((input_face_embeddings[n].unsqueeze(0).cuda()),
                          hair2_t, resize=True)
                    start_output_im = utils_hairstyle.torch_to_numpy_uint8(start_result_batch)
                    end_output_im = utils_hairstyle.torch_to_numpy_uint8(end_result_batch)
                    ...
                    if alpha_blend:
                        ...
                        im_mask1 = utils_hairstyle.gaussian_filter(mask1, sigma=(self.alpha_blend_sigma))
                        im_mask2 = utils_hairstyle.gaussian_filter(mask2, sigma=(self.alpha_blend_sigma))
                        masked_output_A = utils_hairstyle.alpha_blend_images(start_output_im, img_original_A, im_mask1)
                        masked_output_B = utils_hairstyle.alpha_blend_images(end_output_im, img_original_B, im_mask2)
        ...
        if os.path.exists(self.__prepare_filename(im1_path, None, alpha_blend, kwargs={'phase':'manip',  'dir':direction_name,  'strength':coeffs[0]})):
            ...
            for ii in range(len(coeffs)):
                with torch.no_grad():
                    ...
                    manipulated_img = utils_hairstyle.torch_to_numpy_uint8(manipulated_img)
                    ...
                    if alpha_blend:
                        im_mask = utils_hairstyle.gaussian_filter(mask1, sigma=(self.alpha_blend_sigma))
                        masked_output = utils_hairstyle.alpha_blend_images(manipulated_img, im1_orig, im_mask)
